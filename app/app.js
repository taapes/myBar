...
(function() {
    ...
    app.controller('AdminController', ['$http', '$scope', '$log', "$location", function($http, $scope, $log, $location) {
			$scope.Users = [];
            ...
            $scope.remove = function(username){
                $http.post('/php/removeUser.php', {'username': username})
                        .success(function(response, status, headers, config)
                        {
            ...
            $scope.update = function(username, admin){
            	$http.post('/php/editUser.php', {'username': username, 'admin':admin})
                        .success(function(response, status, headers, config)
                        {
                            if (response.success === 0) {
                                $scope.message = response.message;
    ...
    app.controller('AllBarsController', ['$http', '$scope', '$log', "$location", function($http, $scope, $log, $location) {
            ...
            $scope.rating = function() {
                $scope.Bars.forEach(function(bar) {
                    bar.rating = (bar.star1 + (bar.star2 * 2) + (bar.star3 * 3) + (bar.star4 * 4) + (bar.star5 * 5)) / (bar.reviews.length);
                });
            };
            ...
            $scope.getResultsPage = function(pageNumber) {
                ...
                $http.get('/php/getBars.php?page=' + pageNumber)
                        .success(function(response, status, headers, config)
                        {
                            ...
                            console.info(response.message + response.total + response.bars);
                        })
                        .error(function(response, status, headers, config)
                        {
                            alert("Error: " + response.message);
    ...
    app.controller('ViewBarController', ['$scope', '$http', '$routeParams', '$location', 'toaster', '$rootScope', function($scope, $http, $routeParams, $location, toaster, $rootScope) {
            ...
            $scope.removeReview = function(review, rating) {
